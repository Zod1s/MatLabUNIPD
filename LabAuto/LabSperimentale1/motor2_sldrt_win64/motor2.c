...
void motor2_output(void)
{
   ...
   *  Gain: '<Root>/Gain24'
   */
  rtb_Integrator1 = motor2_P.ff.en * motor2_B.Integrator2 * motor2_P.rpm2rads;
  ...
  /* Step: '<Root>/Step' */
  if (motor2_M->Timing.t[0] < motor2_P.Step_Time) {
    tmp = motor2_P.Step_Y0;
  } else {
    tmp = motor2_P.Step_YFinal;
  }

  /* Sum: '<Root>/Sum3' incorporates:
   *  Step: '<Root>/Step'
   */
  motor2_B.Sum3 = tmp - motor2_B.Gain9;

  /* Gain: '<Root>/Gain12' */
  rtb_Gain12 = motor2_P.rpm2rads * motor2_B.Sum3;

  /* Signum: '<Root>/Sign1' */
  if (rtIsNaN(rtb_Integrator1)) {
    tmp = (rtNaN);
  } else if (rtb_Integrator1 < 0.0) {
    tmp = -1.0;
  } else {
    tmp = (rtb_Integrator1 > 0.0);
  }
   ...
   *  Sum: '<Root>/Sum8'
   */
  motor2_B.Sum9 = ((motor2_P.ff.viscous * rtb_Integrator1 + motor2_P.ff.coulomb *
                    tmp) * motor2_P.ff.friction + ((motor2_P.pid.Kp * rtb_Gain12
    + motor2_X.Integrator1_CSTATE) + motor2_P.ff.BEMF * rtb_Integrator1)) +
    motor2_B.Gain22;
...
RT_MODEL_motor2_T *motor2(void)
{
  ...
  rtmSetTFinal(motor2_M, 20.0);
  motor2_M->Timing.stepSize0 = 0.001;
  motor2_M->Timing.stepSize1 = 0.001;
  motor2_M->Timing.stepSize2 = 0.5;

  /* External mode info */
  motor2_M->Sizes.checksums[0] = (2144321464U);
  motor2_M->Sizes.checksums[1] = (2772445699U);
  motor2_M->Sizes.checksums[2] = (1583218060U);
  motor2_M->Sizes.checksums[3] = (3544985256U);
  ...
  motor2_M->Sizes.numBlocks = (52);    /* Number of blocks */
  motor2_M->Sizes.numBlockIO = (12);   /* Number of block outputs */
  motor2_M->Sizes.numBlockPrms = (48); /* Sum of parameter "widths" */
  return motor2_M;
}
